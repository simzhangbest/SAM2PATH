...
class PromptSAMLateFusion(nn.Module):
    def __init__(
        ...
        super().__init__()
        # sim todo here 替换为sam2
        # # build_sam2 
        # self.model2 = build_sam2("sam2", args.sam_ckpt, device="cuda")
        self.model = sam_model_registry[model_type](checkpoint=checkpoint)
        self.mask_HW = mask_HW
        self.feature_input = feature_input
    ...
    def forward(self, images):
        ...
        if not self.feature_input:
            ...
            with torch.no_grad():
                image_embeddings = self.model.image_encoder(images, no_neck=True)  # torch.Size([1, 64, 64, 768])
